...
1. Sign in at huggingface.co, open https://huggingface.co/google/gemma-3-12b-it and click
...
```bash
# 0. Confirm the GPU is an Radeon PRO W7900 and see memory
...
# 2. Serve Gemma 3 12B multimodal (vLLM is preinstalled)
...
r=requests.post("http://localhost:8000/v1/chat/completions", json={
  "model":"google/gemma-3-12b-it","max_tokens":200,"temperature":0.2,
  "messages":[{"role":"user","content":[
    {"type":"image_url","image_url":{"url":f"data:image/jpeg;base64,{b64}"}},
    {"type":"text","text":"Describe what is happening in this video frame in 2 sentences."}]}]})
print(r.json()["choices"][0]["message"]["content"])
PY
#    screenshot this description -> proof Gemma 3 does the video understanding on Radeon PRO W7900.
```
...
```bash
cd stylereel
GEMMA_ENDPOINT="<proxied vLLM /v1 url>" GEMMA_MODEL="google/gemma-3-12b-it" \
...
export GEMMA_ENDPOINT=http://localhost:8000/v1 GEMMA_MODEL=google/gemma-3-12b-it
uv run python harness/ablation.py --n 6 --arms full,blind
```

## Evidence to capture (screenshot each)
- [ ] `rocm-smi` showing the Radeon PRO W7900 and utilisation during a describe call
- [ ] vLLM startup log: `google/gemma-3-12b-it` loaded + vision tower + `--limit-mm-per-prompt image=8`
...
- If 12B download/serve is too slow for the window, fall back to `google/gemma-3-12b-it`
